...
## Key Insights (Korean Summary)

- **개성이 중요**: 에이전트에 이름과 성격을 부여하면 "팀과 대화"하는 느낌
- **공유 메모리 + 개별 컨텍스트**: 공통 목표는 공유하되, 각자 전문 영역 축적
- **작업에 맞는 모델**: 단순 모니터링에 비싼 모델 쓰지 말 것
- **스케줄 작업이 핵심**: 요청 없이도 인사이트를 자동으로 제공
- **2개부터 시작**: 리드 + 전문가 1명으로 시작, 병목 생기면 추가
